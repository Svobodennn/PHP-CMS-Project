...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <form id="project">
                <div class="card-body">
                    <input type="hidden" id="projectId" value="<?= $data['project']['id'] ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="<?= $data['project']['title'] ?>"
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" rows="4" cols="50" id="projectDetails"
                                  name="projectDetails"><?= $data['project']['description'] ?></textarea>
                    </div>
                    <div class="form-group">
                        <label for="projectStartDate">Start Date</label>
                        <input type="date" class="form-control" value="<?= $data['project']['start_date'] ?>"
                               id="projectStartDate" name="projectStartDate" >
                    </div>
                    <div class="form-group">
                        <label for="projectEndDate">End Date</label>
                        <input type="date" class="form-control" value="<?= $data['project']['end_date'] ?>"
                               id="projectEndDate" name="projectEndDate" >
                    </div>
                    <div class="form-group">
                        <label for="projectProgress">Progress</label>
                        <input type="range" min="0" max="100" class="custom-range" value="<?= $data['project']['progress'] ?>"
                               id="projectProgress" name="projectProgress" >
                    </div>
                    <div class="form-group">
                        <label for="projectStatus">Status</label>
                        <select class="form-control" id="projectStatus">
                            <option <?= $data['project']['status'] == 'a' ? "selected='selected'" : null ?> value="a">Active</option>
                            <option <?= $data['project']['status'] == 'p' ? "selected='selected'" : null ?> value="p">Passive</option>
                        </select>
                    </div>
...
<script>
    ...
    project.addEventListener('submit', (e) => {
        ...
        axios.post('<?= _link('project/edit') ?>', formData)
            .then(res => {
